...
- グループ内で定義されている場合でも、Laravelルートがより適切に認識されます。
- 非Laravelプロジェクト向けのパフォーマンスとメモリの最適化。
- ジェネリックテンプレートタイプの処理を改善します。それらが基底クラスのスコープに渡されるときに制約がない場合。[#2501](https://community.devsense.com/d/2501), [#2504](https://community.devsense.com/d/2504)
- スター付き提案AIモデルが更新されました。
- より多くのWordPressの規約を尊重し、エラーが少なく報告されます。
- IntelliSenseにスタブを追加するコードアクションを追加しました（`php.stubs`設定）。これは、PHPの書籍で定義されている不明な関数や不明なクラス名を修正します。

### 中国語翻訳

拡張機能のUIとIntelliSenseが中国語で表示されます。これは、VSCodeの表示言語が`zh-cn`に設定されている場合に自動的に設定されます。
